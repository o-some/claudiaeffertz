...
class CartItems extends window.StandardEvents.createViewEventElement(HTMLElement) {
  constructor() {
    ...
    this.addEventListener('click', (event) => {
      if (!event.target.closest('[data-ch-cart-retry]') || !this.lastFailedUpdate) return;
      const { line, quantity, name, variantId } = this.lastFailedUpdate;
      this.updateQuantity(line, quantity, event, name, variantId);
    });
  }
  ...
  updateQuantity(line, quantity, event, name, variantId) {
    if (this.pendingLines?.has(String(line))) return;
    ...
    fetch(`${routes.cart_change_url}`, { ...fetchConfig(), ...{ body } })
      ...
      .catch((e) => {
        ...
        const errors = document.getElementById('cart-errors') || document.getElementById('CartDrawer-CartErrors');
        if (errors) errors.textContent = window.cartStrings.error;
        this.lastFailedUpdate = { line, quantity, name, variantId };
  ...
  toggleRetry(show) {
    this.querySelectorAll('[data-ch-cart-retry]').forEach((button) => { button.hidden = !show; });
  }
  ...
  enableLoading(line) {
    ...
    row?.querySelectorAll('button:not([disabled]), input:not([disabled]), select:not([disabled])').forEach((control) => {
      control.dataset.chCartPending = 'true';
  ...
  disableLoading(line) {
    ...
    row?.querySelectorAll('[data-ch-cart-pending]').forEach((control) => {
      control.disabled = false;
      delete control.dataset.chCartPending;
    });
